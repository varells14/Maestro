...
<div class="container-fluid my-4">
    <div class="card shadow">
        <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center py-3">
            <h3 class="m-0 font-weight-bold">Purchase Orders Request</h3>
            <div class="d-flex gap-2">
                ...
                <button class="btn btn-light" data-bs-toggle="modal" data-bs-target="#addRequestModal">
                    <i class="fas fa-plus"></i> Request Purchase Orders
                </button>
            </div>
        </div>
    ...
    <div class="row g-2" id="purchasesContainer">
        @forelse($request as $purchase)
            @php
                // Priority color logic (menyesuaikan kapitalisasi konsisten juga)
                $priorityColor = match(strtoupper($purchase->priority)) {
                    'HIGH' => 'danger',
                    'MEDIUM' => 'warning',
                    'LOW' => 'info',
                    'URGENT' => 'dark',
                ...
                if ($purchase->checker === null || 
                    strtolower($purchase->checker) === 'pending' || 
                    strtolower($purchase->checker) === 'waiting') {
                    $statusBadge = 'warning';
                    $statusText = 'Waiting Check';
                }
                // If checker value contains approved or rejected
                elseif (Str::contains(strtolower($purchase->checker), ['approved', 'rejected'])) {
                    if ($purchase->approved === null || 
                        strtolower($purchase->approved) === 'pending' || 
                        strtolower($purchase->approved) === 'waiting') {
                        $statusBadge = 'info';
                ...
                // Fallback ke status utama jika statusText masih 'Pending'
                if ($statusText === 'Pending' && $purchase->status) {
                    $statusBadge = match(strtolower($purchase->status)) {
                        'approved' => 'success',
                        'pending' => 'warning',
                        'rejected' => 'danger',
                        default => 'secondary',
