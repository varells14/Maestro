...
<div class="container-fluid my-4">
    <div class="card shadow">
        ...
        <div class="card-body py-2">
            ...
            <div class="mb-3">
                <input type="text" id="searchInput" class="form-control form-control-sm" placeholder="Search requests...">
            </div>

            <div class="row g-2" id="requestsContainer">
    @forelse($requests as $request)
        @php
            ...
            if ($request->checker === null || 
                strtolower($request->checker) === 'pending' || 
                strtolower($request->checker) === 'waiting') {
                $statusBadge = 'warning';
                $statusText = 'Waiting Check';
            }
            elseif (Str::contains(strtolower($request->checker), ['approved', 'rejected'])) {
                if ($request->approved === null || 
                    strtolower($request->approved) === 'pending' || 
                    strtolower($request->approved) === 'waiting') {
                    $statusBadge = 'info';
                    $statusText = 'Waiting Approval';
                }
                elseif ($request->approved !== null) {
                    if (Str::contains(strtolower($request->approved), 'approved')) {
                        $statusBadge = 'success';
                        $statusText = 'Approved';
                    } 
                    elseif (Str::contains(strtolower($request->approved), 'reject')) {
                        $statusBadge = 'danger';
                        $statusText = 'Rejected';
                    }
                }
            ...
            if ($statusText === 'Pending' && $request->status) {
        ...
        @endphp
        <div class="col-md-6 col-lg-3 mb-2 request-card-wrapper">
    <div class="card h-100 border-1 border-primary shadow-sm request-card" data-id="{{ $request->id }}">
        <!-- Colored header based on status -->
        <div class="card-header bg-{{ $statusBadge }} bg-opacity-10 border-bottom-0 py-2">
            <div class="d-flex justify-content-between align-items-center">
                <h6 class="card-title mb-0 text-truncate fw-bold" title="{{ $request->request_name }}">
        ...
        <div class="card-body pt-2 pb-2">
            ...
            <div class="d-flex justify-content-between align-items-center mb-2 pb-1 border-bottom">
                ...
                <span class="badge bg-{{ $priorityColor }} small py-1 px-2">{{ $request->priority ?? 'Normal' }}</span>
            </div>
            ...
            <div class="d-flex align-items-center mb-2">
                ...
                <div>
                    ...
                    <p class="mb-0 small fw-medium">{{ $request->project }}</p>
                </div>
            </div>
            ...
            <div class="d-flex align-items-center">
                ...
                <div>
                    ...
                    <p class="mb-0 small fw-medium">{{ $request->request_date ? \Carbon\Carbon::parse($request->request_date)->format('d M Y') : 'N/A' }}</p>
                </div>
            </div>
        </div>

        <div class="card-footer bg-light d-flex justify-content-between align-items-center border-top py-2">
            <!-- Created At Timestamp on the Left -->
            <div class="small text-muted">
                {{ $request->created_at->diffForHumans() }}
            </div>
            <!-- View Details Button on the Right -->
            <button class="btn btn-primary btn-sm view-details rounded-pill px-3 shadow-sm" data-id="{{ $request->id }}">
    ...
    @empty
        <div class="col-12 text-center py-3">
            <div class="empty-state p-3 rounded-3 bg-light">
                ...
                <h5 class="text-muted fw-bold">No Material Requests Found</h5>
            </div>
        </div>
    @endforelse
</div>
